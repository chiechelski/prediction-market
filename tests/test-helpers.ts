...
export const collateralMintKeypair = Keypair.fromSecretKey(
  new Uint8Array(
    JSON.parse(
      fs
        .readFileSync(path.resolve(__dirname, "./keys/collateral-mint.json"))
        .toString(),
    ),
  ),
);
...
export const resolverKeypair = Keypair.fromSecretKey(
  new Uint8Array(
    JSON.parse(
      fs
        .readFileSync(path.resolve(__dirname, "./keys/resolver.json"))
        .toString(),
    ),
  ),
);
...
export const deriveAllowedMint = (
  programId: PublicKey,
  mint: PublicKey,
): PublicKey =>
  PublicKey.findProgramAddressSync(
...
export const deriveUserProfile = (
  programId: PublicKey,
  wallet: PublicKey,
): PublicKey =>
  PublicKey.findProgramAddressSync(
...
export const deriveParimutuelState = (
  programId: PublicKey,
  market: PublicKey,
): PublicKey =>
  PublicKey.findProgramAddressSync(
...
/** One tx: `initialize_market_resolver` for slots `0..resolverPubkeys.length-1`. */
export async function initializeMarketResolverSlots(
